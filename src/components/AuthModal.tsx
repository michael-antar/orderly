    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleOpenChange}>
            ...
            <DialogContent className="sm:max-w-[425px] p-0 bg-transparent border-none shadow-none">
                {view === 'login' && <LoginForm onSuccess={() => setIsOpen(false)} onViewChange={() => setView('signup')} onForgotPasswordClick={() => setView('forgot-password')} />}
                ...
                {view === 'forgot-password' && <ForgotPasswordForm onSuccess={() => setIsOpen(false)} onViewChange={() => setView('login')} />}
            </DialogContent>
        </Dialog>
    );
};