...
export const SortControls = ({
  ...
  const sortOptions = useMemo(() => {
    ...
    const dynamic = categoryDef.field_definitions
      .filter((f) => ['number', 'string', 'boolean', 'select'].includes(f.type))
      .map((f) => ({
        label: f.label,
  ...
  const filterRuleOptions = useMemo(() => {
    ...
    const dynamicFields = categoryDef.field_definitions.filter((f) => Object.keys(TYPE_OPERATORS).includes(f.type));
  ...
  const getSelectOptions = useCallback(
    (fieldKey: string, schemaOptions: string[] = []) => {
      ...
      return Array.from(new Set([...schemaOptions, ...Array.from(itemValues)]));
    },
    [items],
  );
        ...
        <div className="flex flex-col max-h-[75vh]">
          ...
          <div className="p-4 pb-0 flex-shrink 0">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Arrange and filter the list to find what you're looking for.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 items-center gap-4">
              ...
              <div className="col-span-2 flex items-center gap-2">
                <Select value={localSortBy} onValueChange={setLocalSortBy} onOpenChange={handleDescendantOpenChange}>
                  <SelectTrigger id="sort-by-select" className="w-full">
                    <SelectValue placeholder="Sort by..." />
                  </SelectTrigger>

                  <SelectContent onEscapeKeyDown={(e) => e.preventDefault()}>
                    {sortOptions.map((opt) => (
                ...
                <Button variant="ghost" size="icon" onClick={toggleSortDirection}>
                  {localSortAsc ? <ArrowUp className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />}
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            <div className="space-y-4">
              ...
              {/* Add Rule Button */}
              <Button variant="outline" size="sm" onClick={handleAddBlankRule}>
