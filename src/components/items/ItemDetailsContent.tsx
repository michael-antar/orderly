    ...
    <>
      ...
      {item.tags && item.tags.length > 0 && (
        <div className="mt-4 flex gap-3">
          <h4 className="font-semibold text-muted-foreground text-sm">Tags</h4>
      ...
      <div className="grid gap-6 sm:grid-cols-2 mt-6">
        {categoryDef.field_definitions.map((field) => (
          <div key={field.key} className="space-y-1.5">
            <h4 className="text-xs font-medium text-muted-foreground uppercase">{field.label}</h4>
            <div className="text-sm font-medium">{renderPropertyValue(field, item.properties[field.key])}</div>
          </div>
        ))}
      </div>

      {/* Description */}
      {item.description && <p className="mt-6 text-sm whitespace-pre-wrap break-words">{item.description}</p>}
    </>
  );
};
