...
export const DynamicFieldRenderer = ({ field, value, onChange }: Props) => {
  ...
  if (field.type === 'select' && field.options) {
    ...
    const isLegacyValue = stringValue !== '' && !field.options.includes(stringValue);
          ...
          <SelectContent>
            ...
            {isLegacyValue && (
              <SelectItem value={stringValue}>
                {stringValue} <span className="text-muted-foreground text-xs ml-1">(Legacy)</span>
              </SelectItem>
            )}
  ...
  if (field.type === 'location') {
    ...
    return (
      <div className="space-y-2">
        ...
        />
        <p className="text-[0.8rem] text-muted-foreground">Map search coming soon.</p>
      </div>
    );
  }
